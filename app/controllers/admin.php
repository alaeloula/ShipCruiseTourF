...
class admin extends Controller
{
  ...
  public function addType()
  {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      // Make sure errors are empty
      if (empty($data['type_err']) && empty($data['quantite_err']) ) {
  ...
  {
    $navires = $this->adminModel->getNavirevs();
    $data = [
      'navires' => $navires,
    ];
    $this->view('admin/getNavire', $data);
  }
  public function deleteNavire($id)
  {
    
    $navires = $this->adminModel->deleteNavirevs($id);
    if ($navires) {
      flash('delete_nav_message', 'shipe deleted');
      redirect('admin/getNavire/');
    }
    // $this->view('admin/getNavire', $data);
  }
  ...
  {
    $croisiere = $this->adminModel->getCroisiere();
    $traget = $this->adminModel->gettragetvs();
    $data = [
      'croisieres' => $croisiere,
      'traget' => $traget
    ];
    $this->view('admin/getCroisiere', $data);
  }
  public function deleteCroisiere($id)
  {
    
    $navires = $this->adminModel->deleteCroisiere($id);
    flash('delete_cruise_message', 'cruise deleted');
    redirect('admin/getCroisiere/');
    // $this->view('admin/getNavire', $data);
  }
