...
class Adminm{
    ...
    {
        $this->db->query('INSERT INTO `type_chambre`(`id_t`, `type`, `quantite`) VALUES (NULL,:type, :quantite)');
        // Bind values
    ...
    public function deleteNavirevs($id)
    {
    ...
    public function getCroisiere()
    {
        ...
        $this->db->query('SELECT * FROM `croisierevs`');
        $results = $this->db->resultSet();
        return $results;
    }
    public function deleteCroisirere($id)
    {
        $this->db->query('DELETE FROM `navire` WHERE `id_n` = :id');
        // Bind values
        $this->db->bind(':id', $id);    
